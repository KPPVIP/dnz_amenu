...
$(function () {
    ...
    window.addEventListener('message', function (event) {
        ...
        if (event.data.type == "open") {
            ...
            $(".player").click(function () {
                ...
                $('.screenshot').remove() 
                $('.banqq').remove() 
                $.post('https://dnz_amenu/main', JSON.stringify({
                    selected: selected,
                }));
                ...
                $(".kick").click(function() {
                    ...
                    $.post('https://dnz_amenu/action', JSON.stringify({
                        target: this.id,
                        da: 'kick',
                        grund: grundaction
                    }));
            ...
            <div class="anzahl">` + item.amount + `</div>
            <div class="hr"></div>
            ...
            <div class="anzahl"> WAFFEN </div>
            <div class="hr"></div>
            <div class="itemname"></div>
            </div>`)
        }  else if (event.data.type == "invhr") {
            $('.clearinv').append(`<div class="item" style="background-color: grey;">
            <div class="anzahl"> INVENTAR </div>
            <div class="hr"></div>
            <div class="itemname"></div>
            </div>`)
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) {
            $.post('https://dnz_amenu/exit', JSON.stringify({}));
            return
        }
    };
    ...
    $(".leavespec").click(function () {
        $.post('https://dnz_amenu/leavespecpls', JSON.stringify({}));
        return
    })
    ...
    $(".showbans").click(function () {
        ...
        $('#Canvas').append(`<div class="loader" style="margin-left: 45%; margin-top: 25%;" id='`+ ee +`'></div>`)
         ...
         $.post('https://dnz_amenu/banlist', JSON.stringify({ 
         }), function(data){});
    ...
    $(".refresh").click(function () {
        $.post('https://dnz_amenu/refreshplayers', JSON.stringify({}));
        return
    })
...
function entbannen(license){
    $.post('https://dnz_amenu/unban', JSON.stringify({license}));
}